...
import BossRaid from '@/components/game/boss-raid'; // 🐉 보스 레이드 컴포넌트 임포트
...
export default function Home() {
  ...
  const [showBossRaid, setShowBossRaid] = useState(false); // 🐉 보스 레이드 모달 상태
  
  const [allMonsters, setAllMonsters] = useState<Monster[]>([]);

  const { currentUser, login, logout, addScore, incrementCombo, resetCombo, resetGame, feverMode, combo, addCoins } = useGameStore();
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-blue-50 p-4 overflow-hidden relative">
        ...
        <MotionCard delay={0.1}>
          <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 mb-2 drop-shadow-sm text-center">
            BRAIN ACADEMY
          </h1>
        ...
        {/* 학습 콘텐츠 영역 (스크롤 축소) */}
        <div className="w-full max-w-sm space-y-4 mb-6 max-h-[40vh] overflow-y-auto p-2 scrollbar-hide">
        ...
        <div className="w-full max-w-sm flex flex-col gap-3">
          {/* 보스 레이드 대형 버튼 */}
          <button onClick={() => setShowBossRaid(true)} className="w-full bg-gradient-to-r from-red-600 via-orange-600 to-red-600 text-white font-black py-4 rounded-2xl shadow-[0_5px_0_rgba(153,27,27,1)] active:scale-95 active:translate-y-1 transition-all flex items-center justify-center gap-3 animate-pulse">
          ...
          <div className="grid grid-cols-2 gap-3 w-full">
            ...
            <button onClick={() => setShowBattle(true)} className="bg-blue-50 text-blue-900 font-black py-3 rounded-2xl shadow-md border-b-4 border-blue-400 flex flex-col items-center active:scale-95 transition-transform"><span className="text-xl">⚔️</span><span className="text-xs mt-1">유저 대결</span></button>
          </div>
        </div>
