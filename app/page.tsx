...
export default function Home() {
  ...
  const enterBossRaid = () => {
    if (currentUser.coins >= 1) {
      if(window.confirm("보스 레이드에 입장할까요? (-1 코인)")) {
        spendCoins(1);
        setShowBossRaid(true);
      }
    } else {
      alert("코인이 부족해! 수학 문제를 더 풀고 코인을 모아오자 📚");
    }
  };
  ...
  if (!gameMode) {
    return (
      <main className="flex min-h-screen flex-col items-center bg-blue-50/50 p-4 pb-12 overflow-x-hidden">
        
        {/* 상단 프로필 바 */}
        <div className="w-full max-w-md flex justify-between items-center bg-white p-3 rounded-full shadow-sm mb-6 mt-2 border border-blue-100">
        ...
        <MotionCard delay={0.1} className="w-full max-w-md mb-6">
        ...
        {/* ⭐️ 압축된 일일 미션 UI */}
        <div className="w-full max-w-md bg-white rounded-3xl shadow-sm p-5 mb-6 border border-gray-100">
          <div className="flex justify-between items-end mb-3">
            <h3 className="font-black text-indigo-900 text-lg flex items-center gap-2">
              <span>📅</span> 오늘의 미션
            </h3>
            <span className="font-bold text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full text-sm">
              {dailySolvedCount}개 성공!
            </span>
          </div>
                ...
                return (
                  <div key={mission.step} className="flex flex-col items-center group relative cursor-pointer" onClick={() => claimMissionReward(mission.step, mission.reward)}>
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm z-10 transition-all ${
                      isClaimed ? 'bg-gray-200 text-gray-400 ring-2 ring-white' : 
                      isCleared ? 'bg-yellow-400 text-white ring-4 ring-yellow-200 animate-bounce shadow-lg' : 
                      'bg-white text-gray-300 ring-2 ring-gray-200'
                    }`}>
                      {isClaimed ? '✔️' : '🎁'}
                    </div>
                    {isCleared && !isClaimed && (
                      <div className="absolute -bottom-6 bg-red-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">
                        +{mission.reward} 받기!
                      </div>
                    )}
                  </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-green-100 flex items-center">
              ...
              <div className="flex-1 grid grid-cols-3 gap-2">
                <button onClick={() => startGame('ADD', 'LEVEL_1')} className="bg-green-50 hover:bg-green-100 text-green-700 py-3 rounded-xl font-black text-sm transition">1단계</button>
                <button onClick={() => startGame('ADD', 'LEVEL_2')} className="bg-green-100 hover:bg-green-200 text-green-800 py-3 rounded-xl font-black text-sm transition">2단계</button>
                <button onClick={() => startGame('ADD', 'LEVEL_3')} className="bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-black text-sm shadow-sm transition">3단계</button>
              </div>
            </div>
            {/* 뺄셈 */}
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-orange-100 flex items-center">
              <div className="w-16 text-center text-2xl drop-shadow-md">➖</div>
              <div className="flex-1 grid grid-cols-3 gap-2">
                <button onClick={() => startGame('SUB', 'LEVEL_1')} className="bg-orange-50 hover:bg-orange-100 text-orange-700 py-3 rounded-xl font-black text-sm transition">1단계</button>
                <button onClick={() => startGame('SUB', 'LEVEL_2')} className="bg-orange-100 hover:bg-orange-200 text-orange-800 py-3 rounded-xl font-black text-sm transition">2단계</button>
                <button onClick={() => startGame('SUB', 'LEVEL_3')} className="bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl font-black text-sm shadow-sm transition">3단계</button>
              </div>
            </div>
            {/* 구구단 */}
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-purple-100 flex items-center">
              ...
              <div className="flex-1 grid grid-cols-3 gap-2">
                <button onClick={() => startGame('MUL', 'LEVEL_1')} className="bg-purple-50 hover:bg-purple-100 text-purple-700 py-3 rounded-xl font-black text-sm transition">1단계</button>
                <button onClick={() => startGame('MUL', 'LEVEL_2')} className="bg-purple-100 hover:bg-purple-200 text-purple-800 py-3 rounded-xl font-black text-sm transition">2단계</button>
                <button onClick={() => startGame('MUL', 'LEVEL_3')} className="bg-purple-500 hover:bg-purple-600 text-white py-3 rounded-xl font-black text-sm shadow-sm transition">3단계</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="grid grid-cols-2 gap-3 w-full">
            <button onClick={enterBattle} className="bg-blue-500 text-white font-black py-4 rounded-3xl shadow-[0_4px_0_rgba(29,78,216,1)] active:translate-y-1 active:shadow-none transition-all flex flex-col items-center gap-1">
              <span className="text-3xl">⚔️</span>
              <span className="text-base mt-1">유저 대결</span>
              <span className="text-[10px] bg-blue-700/50 px-2 py-0.5 rounded-full">🪙 -1</span>
            </button>
            <button onClick={() => setShowGachaShop(true)} className="bg-white text-gray-800 font-black py-4 rounded-3xl shadow-[0_4px_0_rgba(209,213,219,1)] active:translate-y-1 active:shadow-none border border-gray-200 transition-all flex flex-col items-center gap-1">
              ...
              <span className="text-base mt-1">뽑기 상점</span>
              <span className="text-[10px] text-gray-400">운 시험하기</span>
            </button>
            <button onClick={() => setShowMergeLab(true)} className="bg-gray-800 text-white font-black py-4 rounded-3xl shadow-[0_4px_0_rgba(31,41,55,1)] active:translate-y-1 active:shadow-none transition-all flex flex-col items-center gap-1">
              <span className="text-3xl">🧪</span>
              <span className="text-base mt-1 text-indigo-200">합성소</span>
              <span className="text-[10px] text-gray-400">돌연변이 진화</span>
            </button>
            <button onClick={() => setShowCollection(true)} className="bg-white text-gray-800 font-black py-4 rounded-3xl shadow-[0_4px_0_rgba(209,213,219,1)] active:translate-y-1 active:shadow-none border border-gray-200 transition-all flex flex-col items-center gap-1">
              <span className="text-3xl">📖</span>
              <span className="text-base mt-1">내 도감</span>
              <span className="text-[10px] text-gray-400">수집률 확인</span>
            </button>
          </div>
        </div>
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center justify-center p-4 transition-colors duration-500 ${feverMode ? 'bg-red-50' : 'bg-[#FFFDF5]'}`}>
      ...
      <button onClick={() => setGameMode(null)} className="mt-8 text-gray-500 hover:text-gray-800 font-bold text-sm bg-gray-200/50 px-6 py-3 rounded-full transition-all active:scale-95">
        ← 마을로 돌아가기
      </button>
    </main>
  );
